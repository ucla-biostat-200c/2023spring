...
## Deviance
- The Deviance, also called likelihood (ratio) statistics, is
...
## Example: Deviance for a Nomral linear model
...
- For a **saturated model** is to use $y_i$ as the predicted model therefore
$$
\ell(\mathbf{b}_{max}) = -\frac{n}{2}\log(2\pi \sigma^2)
$$

- For **a proposal model** with $p<n$ parameters, the maximum likelihood estimators are 
$$
...
$$
r_{dres_i} = sign(y_i - m_i\hat{p}_i)\sqrt{y_i\ln\left(\frac{y}{m_i\hat{p}_i}\right) + (m_i-y_i)\ln\left(\frac{m_i-y_i}{m_i-m_i\hat{p_i}}\right)}
$$
